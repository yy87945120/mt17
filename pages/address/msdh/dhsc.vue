<template>
	<view class="content grayBackground" style='padding:24upx 28upx 88upx;'>
		<view class='shadow' style='position:relative;text-align:center;padding:48upx 0upx 28upx'>
			...
			<view style='color:#888888;font-size:28upx;margin-top:31upx'>长按识别二维码导航</view>
			
			<view style='color:#C7C7C7;font-size:24upx;margin-top:21upx;'>地址名称：{{companyInformation.addressName}}</view>
			...
			<view @click='collection()' style='position:absolute;right: 33upx;top:48upx;'>
		...
		<view class='dhm_information shadow' style='display:flex;flex-direction: column;justify-content: space-between;margin-top:24upx;box-sizing:border-box;padding:49upx 46upx 60upx 46upx;'>
			<view class='dhm_form'>
			...
			<view class='dhm_form'>
				<view class='dhm_form_item1' style="">地址类别/行业</view>
			...
			<view style='color:#949494;font-size:28upx;text-align: center;margin-top:28upx'>
				此号码将呈现在二维码，别人扫码后可以看<br>到，建议使用虚拟电话
			</view>
			<button @click="buyVirtualPhone()" class='normalButton' style='margin-top:28upx'>我要使用虚拟电话</button>
			<button @click="toMine()" class='normalButton' style='margin-top:28upx'>查看我的导航码</button>
		...
		<view class='shadow' style='display:flex;flex-direction: column;justify-content: space-between;padding:49upx 46upx 60upx 46upx;box-sizing: border-box;margin-top:24upx'>
			...
			<view v-show='companyInformation.advImgs.length > 0'>
...
<script>
	...
	export default {
		components:{uniIcon,linkUrl,banner,contact},
		data() {
			return {
				title: 'Hello',
				collectionI:false,
		...
		methods: {
			checkVirtualPhone(){
				let _this = this;
				this.$ajax("/my/hasVirtualService","POST",{phone:_this.companyInformation.realPhone}).then((res)=>{
					this.companyInformation.virtualPhone = res;
				})	
			},
			toMine(){
				uni.navigateTo({
					"url":"/pages/address/msdh/dhm1?qcode="+this.qrcode
				})
				
			},
			buyVirtualPhone(){
				let _this = this;
				if(_this.companyInformation.useVirtualPhone == 0){
					uni.navigateTo({
						"url":"/pages/phone/contactway?phone="+_this.companyInformation.realPhone
					})
				}else{
					uni.showModal({
						showCancel:false,
			...
			getQcode(){
				...
				this.$ajax("/nav/detail","POST",{qrcodeNo:_this.qrcode}).then((res)=>{
					...
					if(_this.companyInformation.virtualPhone){
						uni.showModal({
							showCancel:false,
							title:"虚拟电话",
							content:"此电话已经开通了虚拟电话，他人扫码将无法获取你的真实号码",
